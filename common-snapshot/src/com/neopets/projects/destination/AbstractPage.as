...
package com.neopets.projects.destination
{
	...
	public class AbstractPage extends Sprite {
		...
		//CONSTRUCTOR
		//----------------------------------------
		public function AbstractPage(pName:String = null, pView:Object = null) 
		{
			if (pName != null) this.name = pName;
			if (pView != null)
			{
				mView = pView
				mView.addEventListener(AbstractDestinationView.OBJ_CLICKED, handleObjClick, false, 0, true);
			}
		}		
		...
		{  
			if (mView.hasEventListener(AbstractDestinationView.OBJ_CLICKED))
			{
		 ...
		 **/
		protected function placeImageButton (buttonClass:String, pName:String, px:Number, py:Number, pAngle:Number = 0, pFrame:String = null):void
		{
		...
		// for consistancy reasons, addImageButton should be called instead of placeImageButton
		protected function addImageButton (buttonClass:String, pName:String, px:Number, py:Number, pAngle:Number = 0, pFrame:String = null)
		{
		...
		{
			//trace ("\n", name, "\n===FROM THE PAGE===========\n", e.oData.DATA.parent.name, e.oData.DATA.parent, "\n")

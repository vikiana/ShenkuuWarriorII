...
package com.neopets.games.marketing.destination.destination_test.page
{
	...
	public class MainPage extends PageDestinationBase {
		...
		public function MainPage(pName:String = null) 
		{
			...
			// Clive added to this class 3/10
			addAssetToList( "workingPrep.swf", "MainPageAsset");// grab swf from sponsors/testdestination/artAssets
			addAssetToList( "FeedAPetAssets.swf", "FeedAPetAssets"); // grab swf from sponsors/testdestination/artAssets
			loadList();
		}		
		...
		protected function setupDisplay():void
		{
			...
			placeTextButton("MC_navButton", "trivia", "Trivia", 10, 2, 0, "out");
			placeTextButton("MC_navButton", "video", "Video", 190, 2, 0, "out");
			placeTextButton("MC_navButton", "info", "Info", 390, 2, 0, "out");
			placeTextButton("MC_textButton", "inst", "Instructions", 390, 50, 0, "out");
			
			// New nav btn - 3/10
			placeTextButton("MC_navButton", "feed", "Feed a Pet", 550, 2, 00, "out");
		...
		override protected function handleObjClick(e:CustomEvent):void
		{
			switch (e.oData.DATA.parent.name)
			{
				...
				case "video":
					...
					var videoPage:VideoPopup = new VideoPopup (); // class in same package
					addChild(videoPage);
					break;
				...
				case "urlButton":
					trace ("Go to website")
					NeoTracker.processClickURL(15760); // test Sony video
					break;

...
package com.neopets.games.marketing.destination.capriSun.disrespectoids.widgets.feed
{
	...
	public class FeedCaller extends BroadcasterClip
	{
		...
		// This function tries to find a clip's art work by removing wrapper layers.
		
		public static function getLoneDescendant(container:DisplayObjectContainer):DisplayObject {
			if(container == null) return null; // abort if there's no container
			// if we have more or less than a single child, this isn't a wrapper.
			var child:DisplayObject;
			if(container.numChildren == 1) {
				// If this is a wrapper, check if it's lone child has contents.
				child = container.getChildAt(0);
				if(child is DisplayObjectContainer) {
					return getLoneDescendant(child as DisplayObjectContainer);
				} else return child;
		...
		protected function onFeedDone(ev:Event) {
			...
			// clear food
			if(_foodClip != null) _foodClip.content = null;

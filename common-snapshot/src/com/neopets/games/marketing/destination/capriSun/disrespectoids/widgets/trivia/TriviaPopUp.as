...
package com.neopets.games.marketing.destination.capriSun.disrespectoids.widgets.trivia
{
	...
	import com.neopets.games.marketing.destination.capriSun.disrespectoids.widgets.TriviaCallButton;
	...
	public class TriviaPopUp extends BasicPopUp
	{
		...
		protected function onAnswerSelected(ev:CustomEvent) {
			...
			// send out tracking
			broadcast(DestinationView.ADLINK_REQUEST,"GameArea to Daily trivia games played");
			broadcast(DestinationView.NEOCONTENT_TRACKING_REQUEST,16061);
			// let php know we've taken a step to completing a task
			responder = new Responder(onRecordResult,onRecordFault);
			delegate.callRemoteMethod("CapriSun2010Service.recordActivity",responder,"act4");
		}
		...
		*/
		public function onAnswerResult(msg:Object):void {
			...
			if("code" in msg) {
				...
				// send out tracking
				broadcast(DestinationView.ADLINK_REQUEST,"GameArea to Daily trivia games incorrect");
				broadcast(DestinationView.NEOCONTENT_TRACKING_REQUEST,16063);
			} else {
				//runJavaScript(GOOD_ANSWER_SCRIPT);
				if(msg is String) showMessage(msg as String);
				else showMessage(SUCCESS_MSG);
				// send out tracking
				broadcast(DestinationView.ADLINK_REQUEST,"GameArea to Daily trivia games correct");
				broadcast(DestinationView.NEOCONTENT_TRACKING_REQUEST,16062);
			}
			broadcast(DestinationView.SHOP_AWARD_REQUEST,"5");
		}

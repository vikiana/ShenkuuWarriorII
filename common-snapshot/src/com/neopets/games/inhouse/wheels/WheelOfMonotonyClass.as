...
package com.neopets.games.inhouse.wheels {
	...
	public class WheelOfMonotonyClass extends MovieClip {
		...
		private var responder2:Responder;
		...
		var monotonyTimer: Timer;		
		var halfMonotonyTimer: Timer;
		
		
		
		
		public function WheelOfMonotonyClass() {
			trace("FUNCTION::WheelOfMonotonyClass");
			super();						
			setTransID(804);
			setWheelID("5"); //1 = knowledge, 2 = excitement, 3 = mediocrity, 4 = misfortune, 5 = monotony
			setSlots(16);
		...
		public function init() {
			...
			//get the translation
			trace("FUNCTION::init :: getting translation");
		...
		public function goWheel(): void {
			...
			responder2 = new Responder(onStartupResult, onFault);
		...
		public function sendData(e:MouseEvent):void {
			...
			responder2 = new Responder(onStartupResult, onFault);
			connection.call("WheelService.startMonotony", responder2 );
		...
		public function wrapUp(e:TimerEvent) {
			trace("Sending WrapUp Data to AMFPHP");
			monotonyTimer.removeEventListener(TimerEvent.TIMER, wrapUp);
			//mcWheel.removeEventListener(MouseEvent.CLICK, sendData);
			//mcWheel.buttonMode = false;
			// Get the data from the input field
			var _params = _wheelID;
		...
		private function onStartupResult(result:Number):void {
		...
		private function startTimer(time:Number): void {			
			trace("TIMER SET: " + time + " SECONDS");
			var milliTime: Number = time * 1000;
			monotonyTimer = new Timer(milliTime,1);
			monotonyTimer.addEventListener(TimerEvent.TIMER, wrapUp);
			monotonyTimer.start();
			var halfMilliTime: Number = time * 500;
			halfMonotonyTimer = new Timer(halfMilliTime,1);
			halfMonotonyTimer.addEventListener(TimerEvent.TIMER, boredReaction);
			halfMonotonyTimer.start();
		}
		...
		private function onResult(result:Object):void {
			checkTimer.removeEventListener(TimerEvent.TIMER, onTimeOut);
			if (result.redirect) {
				trace ("REDIRECT: " + result.redirect);
		...
		private function onPrizeDisplay(evt: Event)  {
			...
			if (_redirectURL) {
				trace("Redirected!");
				var request:URLRequest = new URLRequest(_redirectURL);
		...
		public function boredReaction(e:TimerEvent) {			
			trace("BORED REACTION TRIGGERED");
			halfMonotonyTimer.removeEventListener(TimerEvent.TIMER, boredReaction);

...
package com.neopets.games.inhouse.BubbleMaster
{
	...
	public class Bubblemaster_Document extends NP9_DocumentExtension
	{
		...
		private var mBubblemaster_SetUp:Bubblemaster_SetUp;
		...
		public function Bubblemaster_Document():void{
		...
		{
			if (evt.oData.CMD == mBubblemaster_SetUp.GAME_ENGINE_CLEANED)
			{
		...
		**/
		...
		protected override function gameEngineUpdate():void
		{
			...
			mBubblemaster_SetUp.init(this,mcBIOS.finalPathway, mTranslationData);	
		...
		{
			mBubblemaster_SetUp = new Bubblemaster_SetUp();
			mBubblemaster_SetUp.addEventListener(mBubblemaster_SetUp.SEND_THROUGH_CMD,sendThroughEvent,false,0,true);	
			...
			mcBIOS.game_id =  1213; 														//This Number will be provided by Neopets, For Demo it's 13000, GameEngineTest is 1105
			mcBIOS.game_lang = "en"; 														//Do Not Change
			
			// ** Known bug - If the meter is in the same place as where previous btns used to be,
			// The restart game link will not work and the game will go to the wrong scene
			mcBIOS.meterX = 175;															//This will be the location of the Score Reporting Window From Neopets
			mcBIOS.meterY = 170;   															//This will be the location of the Score Reporting Window From Neopets
			...
			mcBIOS.iBIOSWidth = 640;											//Width of the Game

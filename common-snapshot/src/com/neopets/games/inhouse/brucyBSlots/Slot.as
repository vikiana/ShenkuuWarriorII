...
package com.neopets.games.inhouse.brucyBSlots
{
	...
	public class Slot extends MovieClip {
		...
		//CONSTRUCTOR 
		//----------------------------------------
		public function Slot(pMainGame:Object, pDir:String = UP):void {
		...
		public function substitude(position:int, itemNumber:int):void
		{
			...
			if(xml.hasOwnProperty("slotPadding")) 
			{ 
				padding = Number(xml.slotPadding.@betweenVerticalItems);
			}
			...
			item.y = position * (itemHeight + padding + offset)* stackFactor ;
			 trace ("total offset:" + itemHeight + padding + offset);
			...
			if(item.item != null)
			{
				...
				//item.item.coinText.text = itemNumber
			}
		...
		private function makeSlot():void {
			...
			xml = mMainGame.configXML.GAME;
			
			itemHeight = Number(xml.ArtAssetSize.@itemHeight);
			itemWidth = Number(xml.ArtAssetSize.@itemWidth);
			...
			for (var i:int = 0; i < mItemsArray.length * mRolls; i++) {
				...
				item.y = i * (itemHeight + padding + offset)* stackFactor  ;
				 trace ("total offset:" + itemHeight + padding + offset);

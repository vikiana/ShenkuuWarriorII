...
package com.neopets.games.inhouse.brucyBSlots
{
	...
	public class SlotMachineCore extends MovieClip
	{
		...
		public function init(pSlots:int = 5):void
		{
			...
			var itemWidth:Number = Number(xml.ArtAssetSize.@itemWidth) +  Number(xml.slotPadding.@betweenVerticalItems);
			var itemHeight:Number = Number(xml.ArtAssetSize.@itemHeight) +  Number(xml.slotPadding.@betweenHorizontalItems);
			...
			mSlotContainer.x = 152;
			mSlotContainer.y = 318
			mSlotStopped = 0;
			...
			//frame
			var leverBackground:MovieClip
			if (xml.hasOwnProperty("leverBackground"))
			{
				var leverBackgroundClass:Class = mMainGame.assetInfo.getDefinition(xml.leverBackground.@className)as Class;
				leverBackground = new leverBackgroundClass ();
				leverBackground.x = Number(xml.leverBackground.@x);
				leverBackground.y = Number(xml.leverBackground.@y);
			}
			...
			if (leverBackground != null )addChild(leverBackground);
		...
		public function cleanup():void
		{
			...
			//util.removeChildren(mLineManager)
		...
		private function createMessageBoard():void
		{
			...
			mMainGame.setText("messageFont", messageBoard.button.btnTextField, mMainGame.system.getTranslation("IDS_GAME_BUTTON_CLOSE"))
			messageBoard.button.addEventListener(MouseEvent.CLICK,closeMessageBoard);
			messageBoard.button.stop();
		...
		private function showMessageBoard(mssg:String, buttonOn:Boolean):void
		{
			...
			mMainGame.setText("messageFont", mb.messageText, mssg)
			mMainGame.setText("messageFont", mb.button.btnTextField, mMainGame.system.getTranslation("IDS_GAME_BUTTON_CLOSE"))
		...
		private function createSlots(pSlots:int):void
		{
			...
			var maskY:Number = itemHeight * .5 + 20;
			var maskWidth:Number = itemWidth * 3
			var maskHeight:Number = -itemHeight * 4 - 20
			var stackArray:Array = mMainGame.configXML.GAME.gameSetup.@slot.split("||")
			for (var i:int = 0; i < pSlots; i++) {
				var slot:Slot = new Slot (mMainGame, stackArray[i]);	
				slot.x = i * (itemWidth + Number(mMainGame.configXML.GAME.slotPadding.@betweenHorizontalItems));
				slot.y = 0;	
				var slotMask:EasyMask = new EasyMask (mSlotContainer, slot, i * itemWidth + maskX, maskY, maskWidth, maskHeight)	// Each slot needs its own mask.  Otherwise slot MC is too bit to apply filters
		...
		{
			trace ("I am looking at the right thing")
		...
		private function handleTreasureBonus(array:Array):void 
		{
			...
			switch (type)
			{
				...
				Tweener.addTween(messageBoard, {scaleX:.5 , scaleY:.5, alpha:0, time:.8, delay:1.5,onComplete:closeMessageBoard} );
				break;

/*
   3/2010
   This class is in the progress of being converted to AS3 
   
   Used this AS2 to AS3 conversion site, and it worked pretty well: http://www.5etdemi.com/convert/index.php
   
   - It added imports, put everything in a package, added 'public' before vars, changed _x to x, etc.
...
{ 
	...
	//CLASS DECLARATION
	public class AttackOfTheRevengeGame_GameClass extends MovieClip
	
	{
		// game state vars
		public var st_init:Number;
		public var st_gamehelp:Number;
		public var st_level:Number;
		public var st_play:Number;
		public var st_cntdwn1:Number;
		public var st_cntdwn2:Number;
		public var st_cntdwn3:Number;
		public var st_b4nextlevelwait:Number;
		public var st_gameover:Number;
		public var st_endwait:Number;
		public var st_restart:Number;
		public var st_sendscore:Number;
	
		public var game_state:Number;
		
		public var aRR:Array;  // powerup rarities
		public var aGS:Array;  // holds sensitive vars
		public var aFP:Array;  // play field positions
		public var aGF:Array;  // boolean game flags
		public var aCN:Array;  // the cannon classes
		public var aPR:Array;  // the pirates
		public var aEN:Array;  // the enemie classes
		public var aCD:Array;  // collision distances
		public var aAT:Array;  // array of char codes = names of movie clips to be attached
		public var aRP:Array;  // random phrases
		public var aMC:Array;  // keep track of all attached movieclips 
		
		// master chief
		public var chief:Object;
		public var jacques:Object;
		
		// key codes
		public var kShift:Number;
		
		// in game help screen
		public var bShowGameHelp:Boolean;
		
		// ---------------------------------------------------------------
		// constructor
		// ---------------------------------------------------------------
		public function AttackOfTheRevenge_GameClass ()
		{
			this.st_init      = 0;
			this.st_gamehelp  = 2;
			this.st_level     = 4;
		...
		public function attachCannons()
		{
			...
			for ( var i=1; i<=3; i++ ) {
				...
				clip.x = this.aFP[0][cpos].x;
				clip.y = this.aFP[0][cpos].y;
				this.aMC.push( clip );
		...
		// ---------------------------------------------------------------
		protected function showGameHelp()
		{
			var bShow = false;
			
			if ( _global.bGameHelp ) {
				if ( _root.displaylevel <= 3 ) {
					var clip = _root.attachMovie("mcGameHelp","mcGameHelp",9981,{x:50,y:50});
		...
		public function c4( i )
		{
			...
			// show points
			//_root.showScore1 = "-15";
			//_root.scoreTween1.x = c.m.x + 50;
			//_root.scoreTween1.y = c.m.y - 75;
		...
		public function moveChief( x, y )
		{
			...
			if ( y != 0 ) {
				...
				// scale master chief :)
				if ( this.aGS[11] == 0 ) {
					this.chief.m.scaleX = 90;
					this.chief.m.scaleY = 90;
				}
				else {
					this.chief.m.scaleX = 100;
					this.chief.m.scaleY = 100;
				}
			}
		...
		{
			this.chief.m.x = this.aFP[ this.aGS[11] ][ this.aGS[10] ].x;
			this.chief.m.y = this.aFP[ this.aGS[11] ][ this.aGS[10] ].y;
			if ( oldy == this.aGS[11] ) {
		...
		public function rrrAAA()
		{
			...
			if ( e.bActive && !e.bEntered ) {
				...
				_root.scoreTween1.x = this.aFP[0][this.aGS[10]].x;
				_root.scoreTween1.gotoAndPlay(2);
				// reward points
				...
				else
				{
					...
					// using water
						...
						if ( cannon.bBurning ) {
							...
							// show points
							_root.showScore1 = "+5";
							_root.scoreTween1.x = cannon.m.x + 50;
							_root.scoreTween1.y = cannon.m.y - 75;
							_root.scoreTween1.gotoAndPlay(2);
							// give points
							this.xxxx( 5 );
		...
		public function y_y( t )
		{
			...
			for ( var i=0; i<this.aCN.length; i++ ) {
				...
				if ( c.bDead ) continue;
					...
					if ( t >= c.tEnd ) {
						var cf = c.m.ignition.currentFrame;
		...
		{
			// _level100.include.gameMsg(0,666); // NEEDS TO BE CHANGED USING AS3 SYNTAX

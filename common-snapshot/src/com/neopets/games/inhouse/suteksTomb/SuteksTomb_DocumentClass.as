...
package com.neopets.games.inhouse.suteksTomb
{
	...
	public class SuteksTomb_DocumentClass extends NP9_DocumentExtension
	{
		...
		private var mSuteksTombEngine:SuteksTombEngine;
		...
		public function SuteksTomb_DocumentClass():void{
		...
		{
			if (evt.oData.CMD == mSuteksTombEngine.GAME_ENGINE_CLEANED)
			{
		...
		**/
		...
		protected override function gameEngineUpdate():void
		{
			...
			mSuteksTombEngine.init(this,mcBIOS.finalPathway, mTranslationData);	
		...
		{
			mSuteksTombEngine = new SuteksTombEngine();
			mSuteksTombEngine.addEventListener(mSuteksTombEngine.SEND_THROUGH_CMD,sendThroughEvent,false,0,true);	
			...
			mcBIOS.game_id =  1197; //358 									//This Number will be provided by Neopets, For Demo it's 13000, GameEngineTest is 1105
			mcBIOS.game_lang = "en"; 									//Do Not Change
			mcBIOS.meterX = 200;										//This will be the location of the Score Reporting Window From Neopets
			mcBIOS.meterY = 240;   										//This will be the location of the Score Reporting Window From Neopets

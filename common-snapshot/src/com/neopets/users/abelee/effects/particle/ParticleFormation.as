...
package com.neopets.users.abelee.effects.particle
{
	...
	public class ParticleFormation
	{
		 ...
		 **/
		public static function smoke(pArray:Array, pHolder:Sprite, pImageName:String, px:Number, py:Number, pxvel:Number = 5, pyvel:Number = 8, pxg:Number = 2, pyg:Number = -2, pfric:Number = .05):void
		{
			for (var i:int = 0; i < pArray.length; i++)
			{
				...
				pArray[i].init(pHolder, pImageName, new Point (px,py), new Point(xvel, yvel), new Point (xgravity, ygravity), pfric, 0.03, 10);
			}
		}
		 ...
		 **/
		public static function fireworks(pArray:Array, pHolder:Sprite, pImageName:String, px:Number, py:Number, pRad:int = 15):void
		{
			...
			for (var i:int = 0; i < pArray.length; i++)
			{
				...
				var xgravity:Number = Math.random()*  0
				var ygravity:Number = Math.random() + 1
				
				pArray[i].init(pHolder, pImageName, new Point (px,py), new Point(xvel, yvel), new Point (xgravity, ygravity), 0.12, 0.01, 15);
			}

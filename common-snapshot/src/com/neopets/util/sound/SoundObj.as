package com.neopets.util.sound
{
	...
	public class SoundObj extends Sound implements ISoundObject
	{
		...
		public const SOUND_BUFFERED:String = "SoundisBuffered";
		public const SOUND_FINISHED:String = "SoundisFinished";
		public const SOUND_TIMEDISPLAY:String = "SoundTCUpdate";
		public const SOUND_LOADED:String = "SoundisLoaded";
		 ...
		 * * **/
		 ...
		 public function SoundObj(pSoundName:String = "",pType:String = "SoundFileisinLibrary",pAmountToBuffer:Number = 0,pLocation:String = null,pPool:String = null, pVolume:Number = 1,pApplicationDomain:ApplicationDomain = null):void
		 {
				...
				{
					var request:URLRequest = new URLRequest(pLocation);
					addEventListener(IOErrorEvent.IO_ERROR, onError,false,0,true);
					addEventListener(Event.COMPLETE,onComplete,false,0,true);
					
					if (pAmountToBuffer != 0)
					{
						var buffer:SoundLoaderContext = new SoundLoaderContext(pAmountToBuffer);
		...
		*/
		...
		private function onProgressCheck(evt:ProgressEvent):void
		{
			if (this.isBuffering == false)
			{
				...
				dispatchEvent(new Event(SOUND_BUFFERED));
			}	
		}
		...
		/** 
       	...
       	{
       		dispatchEvent(new CustomEvent({ID:name},SOUND_LOADED));
       	}
		...
		{
			if (mCurrentSoundChannel){
				this.dispatchEvent(new CustomEvent({TIME:mCurrentSoundChannel.position},SOUND_TIMEDISPLAY));	
			}

...
package com.neopets.util.array
{
	...
	public class ArrayUtils {
		...
		// This function moves the selected elements out of the source array and onto the end of the destination array.
		public static function transferElements(src:Array,dest:Array,startIndex:int=0,endIndex:int=-1):void {
			if(src == null || dest == null) return;
			// validate indices
			startIndex = getValidIndex(startIndex,src);
			endIndex = getValidIndex(endIndex,src);
			// calculate number of elements to be transferred
			var count:int = endIndex - startIndex + 1;
			if(count < 1) return; // if nothing is being transferred, abort.
			// extract elements from source
			var elements:Array = src.splice(startIndex,uint(count));
			// push extracted elements to destination
			var element:Object;
